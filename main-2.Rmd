...
```{r fig-margin, fig.cap = "MPG vs horsepower, colored by transmission.", fig.fullwidth = TRUE, fig.width=10, fig.height=5, cache=TRUE, message=FALSE}
dt_01 %>%
...
where date(started_at) >= cast('2020-04-01' as date) 
  and date(started_at) <= cast('2021-03-31' as date)
group by member_typ, rideable_type, to_char(date(started_at), 'yy-mm'), date(started_at)
```
